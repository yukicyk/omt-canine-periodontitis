...
#
# DEPENDENCIES: gganimate, gifski, phyloseq, dplyr, ggplot2
#

# --- 1. Load Libraries and Data ---

if (!require("pacman")) install.packages("pacman")
pacman::p_load(phyloseq, ggplot2, dplyr, gganimate, gifski)
...
p_rc <- plot_ordination(rc.ra, mds_rc, "samples", color = "Dog") +
  ...
  labs(
    x = "MDS Axis 1",
    y = "MDS Axis 2",
    title = 'Microbiome Trajectory (Weeks: {frame_time})',
...
animate(
  ...
  width = 1200,
  height = 700,
  renderer = gifski_renderer(file.path(fig_path, "community_trajectory_animated.gif"))
)
